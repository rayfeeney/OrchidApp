	...
	(
		(
			(
				...
				(
					(`loc`.`locationId` = `plh`.`locationId`)
					AND (`loc`.`isActive` = 1)
					)
				)
		) LEFT JOIN `orchids`.`plantphoto` `pp` ON (
			(
				(`pp`.`plantId` = `p`.`plantId`)
				AND (`pp`.`isHero` = 1)
				AND (`pp`.`isActive` = 1)
				)
			)
