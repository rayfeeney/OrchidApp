...
AS WITH plant_identity AS (
    SELECT p.plantId AS plantId
    FROM plant p
    WHERE p.isActive = 1
)
SELECT
    pi.plantId AS plantId,
    pe.eventDateTime AS eventDateTime,
    ...
    CONCAT(
        ...
        CASE
            WHEN f.startDate IS NOT NULL AND f.endDate IS NOT NULL
                THEN CONCAT(' FROM ', DATE_FORMAT(f.startDate,'%d-%m-%Y'), ' to ', DATE_FORMAT(f.endDate,'%d-%m-%Y'))
...
SELECT
    ...
    CONCAT(
        'Moved to ', l.locationName,
        ' on ', DATE_FORMAT(plh.startDateTime,'%d-%m-%Y'),
        CASE
