...
SELECT
    ...
    CASE 
        WHEN EXISTS (
            SELECT 1
            FROM vplantlineage lp
            WHERE lp.childPlantId = p.plantId
        )
        THEN 1 ELSE 0
    END AS hasParent,
    ...
    /* Parent Tag */
    (
        SELECT parent.plantTag
        FROM vplantlineage lp
        JOIN plant parent ON parent.plantId = lp.parentPlantId
        WHERE lp.childPlantId = p.plantId
        LIMIT 1
    ) AS parentPlantTag,
    ...
    (
        SELECT lp.relationshipType
        FROM vplantlineage lp
        WHERE lp.childPlantId = p.plantId
        LIMIT 1
    ) AS parentRelationshipType,

    /* Has Children */
    CASE 
        WHEN EXISTS (
            SELECT 1
            FROM vplantlineage lc
            WHERE lc.parentPlantId = p.plantId
        )
        THEN 1 ELSE 0
    END AS hasChildren
