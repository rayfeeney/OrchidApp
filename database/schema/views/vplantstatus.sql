...
AS `parentPlantTag`,(SELECT `lp`.`relationshipType` FROM `vplantlineage` `lp` WHERE `lp`.`childPlantId` = `p`.`plantId` limit 1)
AS `parentRelationshipType`,case when exists(SELECT 1 FROM `vplantlineage` `lc` WHERE `lc`.`parentPlantId` = `p`.`plantId` limit 1) then 1 else 0 end
