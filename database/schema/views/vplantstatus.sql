...
AS `lastFeedTypeDisplayName`,case when `psc`.`plantSplitChildId` is not null then 1 else 0 end
AS `hasParent`,`ps`.`parentPlantId`
AS `parentPlantId`,`parent`.`plantTag`
AS `parentPlantTag`,case when `children`.`plantSplitId` is not null then 1 else 0 end
